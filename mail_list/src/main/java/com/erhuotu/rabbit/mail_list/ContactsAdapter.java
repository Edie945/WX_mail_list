...
public class ContactsAdapter extends BaseAdapter {
    ...
    public ContactsAdapter(Context context, List<NameBean> list) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        String word = list.get(position).firstLetter;
        holder.tv_word.setText(word);
        holder.tv_name.setText(list.get(position).nameChinese);
        //将相同字母开头的合并在一起
        if (position == 0) {
            //第一个是一定显示的
            ...
            String headerWord = list.get(position - 1).firstLetter;
